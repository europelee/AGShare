 ...
 * 
 * FileExplorer is free software: you can redistribute it and/or modify it under the terms of the
 * GNU General Public License as published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version.
 * 
 * FileExplorer is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without
...
public class FileIconLoader implements Callback {
  ...
  private static class BitmapHolder extends ImageHolder {
    ...
    public boolean setImageView(ImageView v) {
      if (bitmapRef.get() == null) return false;
  ...
  private static class DrawableHolder extends ImageHolder {
    ...
    @Override
    public boolean setImageView(ImageView v) {
      if (drawableRef.get() == null) return false;
  ...
  private final static ConcurrentHashMap<String, ImageHolder> mImageCache =
      new ConcurrentHashMap<String, ImageHolder>();
   ...
   */
  private final ConcurrentHashMap<ImageView, FileId> mPendingRequests =
      new ConcurrentHashMap<ImageView, FileId>();
  ...
  /**
   * Checks if the photo is present in cache. If so, sets the photo on the view, otherwise sets the
   * state of the photo to {@link BitmapHolder#NEEDED}
   */
  ...
  public long getDbId(String path, boolean isVideo) {
    ...
    Uri uri =
        isVideo ? Video.Media.getContentUri(volumeName) : Images.Media.getContentUri(volumeName);
  ...
  public long getAlbumId(String path) {
    ...
    String[] columns = new String[] {MediaStore.Audio.Media.ALBUM_ID};
  ...
  private class LoaderThread extends HandlerThread implements Callback {
        ...
        if (holder != null && holder.state == ImageHolder.NEEDED) {
          ...
          switch (id.mCategory) {
            ...
            case Video:
              ...
              if (id.mId == 0) id.mId = getDbId(id.mPath, isVideo);
              if (id.mId == 0) {
            ...
            case Music:
              if (id.mId == 0) id.mId = getAlbumId(id.mPath);
              if (id.mId == 0) {
                Log.e("FileIconLoader", "Fail to get dababase id for:" + id.mPath);
              }
              holder.setImage(getMusicThumbnail(id.mId));
              break;
            default:
              Log.e("FileIconLoader", "unkown:" + id.mCategory);
              break;
          }
    ...
    private Bitmap getMusicThumbnail(long id) {
      String album = CommUtil.getAlbumArt(id, mContext);
      if (null != album)
        return BitmapFactory.decodeFile(album);
      else
        return null;
    }
  }
