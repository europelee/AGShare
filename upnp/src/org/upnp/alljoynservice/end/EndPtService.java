...
public class EndPtService extends Service  implements ServiceConfig {
	...
	{
		SERVER_END,
		CLIENT_END	//need doublerole future?
	};
	
	public static final String CLIENT = "client";
	public static final String SERVER = "server";
	
	private ServiceRole  mRole;	//ugly, may be better to use inhert and strategy pattern(for bus communication)
	...
	private class AlljoynBusListener extends BusListener {
		...
		public void lostAdvertisedName(String name, short transport,
				String namePrefix) {
			Log.i(TAG, String.format("MyBusListener.lostAdvertisedName(%s, 0x%04x, %s)", name, transport, namePrefix));
			mIsjoinSession = false;
			
		}
		
		public void nameOwnerChanged(String busName,
                String previousOwner,
                String newOwner)
		{
			Log.i(TAG, "nameOwnerChanged: "+"busName:"+busName+" previousOwner:"+previousOwner+" newOwner:"+newOwner);
		}
	}
    ...
    {         
    	...
    	//String sRole = bunde.getString(EndPtService.class.getName(), CLIENT); //for android 3.1 above
	...
	private void doConnect() {
		...
		if (ServiceRole.CLIENT_END == mRole)//ugly
		{
			...
			logStatus(
					"BusAttachement.findAdvertisedName " + mServiceName,
					status);
			if (Status.OK != status) {
		...
		logStatus(String.format(
				"BusAttachment.requestName(%s, 0x%08x)",
				mServiceName, flag), status);
		if (status == Status.OK) {
			status = mBus.advertiseName(mServiceName,
					SessionOpts.TRANSPORT_ANY);
			logStatus(
					"BusAttachement.advertiseName " + mServiceName,
					status);
			if (status != Status.OK) {
				status = mBus.releaseName(mServiceName);
				logStatus(String.format(
						"BusAttachment.releaseName(%s)",
						mServiceName), status);
	...
	private boolean doJoinSession(String name) {
		...
		logStatus("BusAttachment.joinSession() - sessionId: " + sessionId.value + " name:"+name, status);
		...
		mProxyObj = mBus.getProxyBusObject(mServiceName,
				mBusObjPath, sessionId.value,
